...
//  //SET THE THRESHOLD TO SWITCH TO LOCAL COMPUTATION USING SCALA
...
//
//
//  //IN LOCAL COMPUTATIONN, WE ARE SPLITING AND TESTING ON THE ARRAY NOT ON THE RDD
...
//
//      //SWITCH TO LOCAL DELTA DEBUGGING WHEN THE MOVE_TO_LOCAL THRESHOLD IS MET AND IT WAS STILL RUNNING ON CLUSTER
...
//      }
//      //OTHERWISE KEEP RUNNING ON CLUSTER
//
//      println(s""">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> $sizeRDD <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<""")
//
...
//
//      //if (sizeRDD <= 1) {
//      //END DELTA DEBUGGING WHEN THE SIZE IS SMALLER OR EQUAL TO THE DATA_THRESHOLD
